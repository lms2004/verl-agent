...
class GSM8KEnvironmentManager(EnvironmentManagerBase):
    """
    EnvironmentManager for GSM8K environment.
    """
    def __init__(self, envs, projection_f, config):
    ...
    def build_text_obs(
        ...
        for i in range(len(text_obs)):
            if init or self.config.env.history_length <= 0:
                obs_i = GSM8K_TEMPLATE_NO_HIS.format(
                    task_description=self.tasks[i]
                )
            else:
                obs_i = GSM8K_TEMPLATE.format(
                    task_description=self.tasks[i],
                    memory_context=memory_ctx[i],
                    step_count=len(self.memory[i]),
                )
    ...
    def success_evaluator(
        ...
        """Evaluate success rate for GSM8K."""
        ...
        for batch_idx in range(len(total_batch_list)):
            ...
            for i in reversed(range(len(total_batch_list[batch_idx]))):
                ...
                if batch_item['active_masks']:
                    info = total_infos[batch_idx][i]
                    # For GSM8K, success is determined by reward > 0 (correct answer)
                    # The reward is computed by reward_fn, so we check if final reward > 0
                    won_value = float(episode_rewards[batch_idx] > 0)
                    success['success_rate'].append(won_value)
        ...
        for key, values in success.items():
            success_arrays[key] = np.array(values, dtype=np.float32)
        
        return success_arrays
